        ...
        for compartment in compartments.data:
            if compartment.lifecycle_state == "ACTIVE":  
                ...
                # Extract and print instance details
                        for subnets in list_subnets_response.data:
                            subnet_response=subnets.__dict__
                            # print(subnet)
                            check=0
                            # print(subnets)
...
def insert_subnet(subnet_list):
    ...
    try:
        ...
        for n in subnet_list:
            insert_query = """
                INSERT INTO cmdb_ci_cloud_subnet(Name,Object_id,cidr,Domain_name,state,Account_ID,Datacenter,Network_Object_ID,Tags,VM_Object_ID,Vnic_attachment_id) 
                values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);
            """
            try:
                cursor.execute(insert_query,(n['display_name'],n['id'],n['cidr_block'],n['domain_name'],n['State'],n['account_id'],n['Datacenter'],n['Network_Object_ID'],n['Tags'],n['VM_Object_ID'],n['Vnic_attachment_id']))
